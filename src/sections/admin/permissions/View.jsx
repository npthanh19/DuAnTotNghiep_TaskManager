...
export function View() {
    ...
    const handleDeleteChild = async (childId) => {
        try {
            ...
            setChildrenPermissions((prevChildrenPermissions) =>
                prevChildrenPermissions.filter((child) => child.id !== childId)
            );
            ...
            <div className="card-body" style={{ padding: '0' }}>
                                        ...
                                        <ul className="dropdown-menu" aria-labelledby={`dropdownMenuButton${permission.id}`}>
                                            <li>
                                                <button className="dropdown-item text-warning" onClick={() => handleAddChild(permission)}>
                                                    <i className="bi bi-pencil me-2"></i> {t('Add Children')}
                                                </button>
                                            </li>
                                            <li>
                ...
                {isAddingChild && (
                    <div className="form-container card shadow-sm p-4 mt-3">
                        ...
                        <form onSubmit={(e) => { e.preventDefault(); handleCreateChildPermission(); }}>
                            <div className="mb-3">
                            ...
                            <div className="d-flex justify-content-end">
                                <button type="submit" className="btn btn-success btn-sm me-2">{t('Add')}</button>
                                <button
                                    type="button"
                                    className="btn btn-outline-secondary btn-sm"
                                    onClick={() => setIsAddingChild(null)}>
                                    {t('Cancel')}
                                </button>
                            </div>
                        </form>
                ...
                {childrenPermissions.length > 0 && showChildren && (
                    <div className="card mt-3 shadow-sm">
                        <div className="card-header d-flex justify-content-between align-items-center">
                            <h5>{t('Child Permissions of')} {parentPermissionName}</h5>
                            <button
                            ...
                            <table className="table table-striped table-bordered table-hover">
                                ...
                                <tbody>
                                    {childrenPermissions.length === 0 ? (
                                        <tr>
                                            <td colSpan="3" className="text-center">{t('No Children')}</td>
                                        </tr>
                                    ) : (
