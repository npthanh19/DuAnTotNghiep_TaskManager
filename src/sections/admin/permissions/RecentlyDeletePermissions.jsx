...
function RecentlyDeletedPermissions() {
    ...
    const handleRestore = async (id) => {
        const result = await Swal.fire({
            title: t('Khôi phục quyền'),
            text: t('Bạn có chắc chắn muốn khôi phục quyền này?'),
            icon: 'warning',
        ...
        if (result.isConfirmed) {
            try {
                ...
                toast.success(t('Quyền đã được khôi phục thành công.'));
            } catch (error) {
                toast.error(t('Đã xảy ra lỗi khi khôi phục quyền.'));
            }
        }
    };
    ...
    const handleDeletePermanently = async (id) => {
        const result = await Swal.fire({
            title: t('Xóa quyền'),
            text: t('Bạn có chắc chắn muốn xóa vĩnh viễn quyền này?'),
            icon: 'warning',
            showCancelButton: true,
            confirmButtonColor: '#3085d6',
            cancelButtonColor: '#d33',
        ...
        if (result.isConfirmed) {
            try {
                ...
                setPermissions(permissions.filter(permission => permission.id !== id));
                toast.success(t('Quyền đã được xóa vĩnh viễn.'));
            } catch (error) {
    ...
    return (
        <div className="card">
                ...
                <table className="table">
                    ...
                    <tbody>
                                ...
                                <td>
                                    <div className="dropdown">
                                        ...
                                        <ul className="dropdown-menu" aria-labelledby={`dropdownMenuButton${permission.id}`}>
                                            <li>
                                                <button className="dropdown-item text-warning" onClick={() => handleRestore(permission.id)}>
                                                    <i className="bi bi-pencil me-2"></i> {t('Restore')}
                                                </button>
                                            </li>
                                            <li>
                                                <button className="dropdown-item text-danger" onClick={() => handleDeletePermanently(permission.id)}>
                        ...
                        {currentPermissions.length === 0 && (
                            <tr>
                                <td colSpan="3" className="text-center">{t('No recently deleted permissions found.')}</td>
                            </tr>
                        )}
                    </tbody>
                </table>
            ...
            <ToastContainer position="top-right" autoClose={1500} hideProgressBar={false} newestOnTop={false} closeOnClick={false} rtl={false} pauseOnFocusLoss draggable pauseOnHover />
        </div>
    );
}
